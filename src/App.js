...
function App() {
  ...
  const getUser = (username) => {
    fetch(
      `https://api.github.com/users/${encodeURI(username)}?access_token=${
  ...
  if (pageState === states.login)
    return <div>{<LoginForm onLogin={getUser} />}</div>;
      ...
      <div>
        {userData && (
          <PlayerHeader
            ...
            playerImage={userData.image}
            size="large"
          />
        )}
        {/*delete the button*/}
        <button
          onClick={() => setPageState(states.inGame)}
          style={{
            width: "100px",
            height: "100px",
            position: "fixed",
  ...
  if (pageState === states.inGame)
    return (
      <div>
        ...
        {/*delete the button*/}
        <button
          onClick={() => setPageState(states.homePage)}
          style={{
            width: "100px",
            height: "100px",
            position: "fixed",
